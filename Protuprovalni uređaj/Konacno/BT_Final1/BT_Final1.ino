...
Adafruit_NeoPixel strip = Adafruit_NeoPixel(3, 2); char ADDR[13]="446EE5E5504D";
...
void loop()
{
  ...
  if (udaljenost >= 15 && !Inquire(0, ADDR)) {
    tone(12, 1000);
...
void BTRead_INQ() {
  ...
  do {
    ...
    for (i = 0; i < BT_DEVICE_COUNT_MAX; i++) {
        strncpy(DeviceList[i].address, &(output_c[i * 30 + 6]), 12);
        //strncpy(DeviceList[i].rssi, &(output_c[i * 30 + 26], 4);
        DeviceList[i].rssi = strtol(&(output_c[i * 30 + 26]), 0, 16);
    }
}
...
void initBT() {
  ...
  cmd = "AT+INQM=1," + String(BT_DEVICE_COUNT_MAX) + ",5\r\n";
  BTprintR(&cmd);
}
