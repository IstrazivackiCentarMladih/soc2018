...
//  Serial.print(ADDR);
//  Serial.print("To je bila adresa");
...
//  Serial.println(udaljenost); TEST
...
//    if(speedVar == 0){
    if(!Inquire(ADDR)){
      ...
      if(RELAY_STATE == 1){
        RELAY_STATE = 0;
        Serial.println("Ne otvaram i postavljam na 0");
        digitalWrite(LED_PIN, LOW);
      }
      ...
      if(RELAY_STATE == 0){
        ...
        Serial.println("Otvaram i postavljam na 1");
        RELAY_STATE = 1;  
             
      }
      Serial.println("Ne Pistim - otvaram relay!");
    }
  }else {
    noTone(12);
    Serial.println("Ne Pistim");
    RELAY_STATE = 0;
    Serial.println("Ne otvaram i postavljam na 0");
    digitalWrite(LED_PIN, LOW);
  }
  //speedVar = (speedVar+1)%speedVarKonst;
...
// TEST
//    Serial.println("Trenutno sam u INQ funkciji");
//    PrintWthFeed(&output);
...
bool Inquire(char CmpAddr[]) {
  BTRead_INQ();
  //Test
//  Serial.println("Procitana BT Adresa:");
//  Serial.println(INQCheck.address);
  
  if (strncmp(INQCheck.address, CmpAddr, 12) != 0){
